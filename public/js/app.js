(function () {
  ...
  /* ---------------- Ghi nho bai dang lam do: quay lai la lam tiep ----------------
     hyv_resume = { "<lessonUrl>": { "<phan>": { t: ms, v: trang thai } } }
     Moi phan nho trong bai (tu vung tung che do, ngu phap, hoi thoai, nghe, noi,
     game, dich, khoi dong, sach bai tap, on tap nhanh) tu luu trang thai cua minh
     sau moi cau tra loi. v = null nghia la "da xoa" (lam xong / bam lam lai) —
     van giu moc thoi gian de lan dong bo sau xoa luon ban cu tren thiet bi khac.
     Da dang nhap thi gui len may chu (/api/resume/sync) de doi may van lam tiep. */
  var RESUME_KEY = 'hyv_resume';
  var RESUME_TTL = 90 * 864e5;
  var RESUME_TOMBSTONE_TTL = 30 * 864e5;
  var rsDirty = {};
  var rsSyncTimer = null;
  var rsRestoring = 0;   // > 0: dang dung lai bai cu -> khong ghi de, khong phat am thanh
  var sfxMuted = 0;

  function rsStore() { return readJSON(RESUME_KEY, {}); }

  function rsUrl(lesson) {
    var l = lesson || currentHubLesson;
    return l && l.fullPageUrl ? l.fullPageUrl : null;
  }

  function rsGet(part, lesson) {
    var url = rsUrl(lesson);
    if (!url) return null;
    var entry = (rsStore()[url] || {})[part];
    if (!entry || entry.v == null || Date.now() - entry.t > RESUME_TTL) return null;
    return entry.v;
  }

  function rsWrite(part, value, lesson) {
    var url = rsUrl(lesson);
    if (!url || rsRestoring) return;
    var all = rsStore();
    var cur = all[url] && all[url][part];
    if (value == null && (!cur || cur.v == null)) return;
    all[url] = all[url] || {};
    all[url][part] = { t: Date.now(), v: value == null ? null : value };
    writeJSON(RESUME_KEY, all);
    rsDirty[url] = true;
    clearTimeout(rsSyncTimer);
    rsSyncTimer = setTimeout(function () { rsFlush(false); }, 1500);
  }
  function rsSet(part, value, lesson) { rsWrite(part, value, lesson); }
  function rsClear(part, lesson) { rsWrite(part, null, lesson); }

  // Bai nay con phan nao dang lam do bat dau bang 1 trong cac tien to?
  function rsHasPrefix(lesson, prefixes) {
    var url = rsUrl(lesson);
    var parts = (url && rsStore()[url]) || {};
    return Object.keys(parts).some(function (k) {
      var e = parts[k];
      if (!e || e.v == null || Date.now() - e.t > RESUME_TTL) return false;
      return prefixes.some(function (p) { return k === p || k.indexOf(p) === 0; });
    });
  }

  function rsPrune(all) {
    var now = Date.now();
    Object.keys(all).forEach(function (url) {
      var parts = all[url];
      if (!parts || typeof parts !== 'object') { delete all[url]; return; }
      Object.keys(parts).forEach(function (k) {
        var e = parts[k];
        if (!e || typeof e.t !== 'number' || now - e.t > RESUME_TTL || (e.v == null && now - e.t > RESUME_TOMBSTONE_TTL)) delete parts[k];
      });
      if (!Object.keys(parts).length) delete all[url];
    });
    return all;
  }

  function rsFlush(keepalive) {
    var urls = Object.keys(rsDirty);
    var auth = readJSON(STORAGE_KEYS.auth, null);
    if (!urls.length || !auth || !auth.token) return;
    var all = rsStore();
    var payload = {};
    urls.forEach(function (u) { if (all[u]) payload[u] = all[u]; });
    rsDirty = {};
    fetch('/api/resume/sync', {
      method: 'POST',
      keepalive: !!keepalive,
      headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + auth.token },
      body: JSON.stringify({ resumeState: payload })
    }).then(function (r) {
      if (!r.ok) urls.forEach(function (u) { rsDirty[u] = true; });
    }).catch(function () {
      urls.forEach(function (u) { rsDirty[u] = true; });
    });
  }

  // Gop ban tren may chu voi ban tren may nay: moi phan lay ban moi hon.
  // Phan nao may nay moi hon thi danh dau de gui nguoc len.
  function rsMergeFromServer(serverState) {
    if (!serverState || typeof serverState !== 'object') return;
    var local = rsStore();
    Object.keys(serverState).forEach(function (url) {
      var sParts = serverState[url] || {};
      local[url] = local[url] || {};
      Object.keys(sParts).forEach(function (k) {
        var s = sParts[k], l = local[url][k];
        if (s && typeof s.t === 'number' && (!l || s.t > l.t)) local[url][k] = s;
      });
    });
    Object.keys(local).forEach(function (url) {
      var sParts = serverState[url] || {};
      Object.keys(local[url] || {}).forEach(function (k) {
        var s = sParts[k], l = local[url][k];
        if (l && (!s || l.t > s.t)) rsDirty[url] = true;
      });
    });
    writeJSON(RESUME_KEY, rsPrune(local));
    if (Object.keys(rsDirty).length) {
      clearTimeout(rsSyncTimer);
      rsSyncTimer = setTimeout(function () { rsFlush(false); }, 1500);
    }
  }

  // Dung lai bai cu: khong luu de, khong keu tieng dung/sai
  function rsQuiet(fn) {
    rsRestoring++;
    sfxMuted++;
    try { fn(); } finally {
      rsRestoring--;
      setTimeout(function () { sfxMuted--; }, 80);
    }
  }
  ...
  // Dong thong bao "Da khoi phuc bai lam do" ngay tren noi dung phan dang hoc
  function rsNotice(contentSel, text, onRestart) {
    var content = $(contentSel);
    if (!content || !content.parentNode) return;
    rsHideNotice(contentSel);
    var el = document.createElement('div');
    el.className = 'rs-notice';
    el.id = 'rsNotice-' + content.id;
    el.setAttribute('role', 'status');
    el.innerHTML =
      '<span class="rs-notice-icon" aria-hidden="true">↺</span>' +
      '<span class="rs-notice-text">' + text + '</span>' +
      '<button type="button" class="rs-notice-restart">Làm lại từ đầu</button>' +
      '<button type="button" class="rs-notice-close" aria-label="Ẩn thông báo">×</button>';
    content.parentNode.insertBefore(el, content);
    $('.rs-notice-restart', el).addEventListener('click', function () {
      el.remove();
      onRestart();
    });
    $('.rs-notice-close', el).addEventListener('click', function () { el.remove(); });
  }

  function rsHideNotice(contentSel) {
    var content = $(contentSel);
    if (!content) return;
    var old = document.getElementById('rsNotice-' + content.id);
    if (old) old.remove();
  }

  // Chu go dang do (dien tu, dich cau...): luu sau khi ngung go 0,6 giay
  var rsTypeTimers = {};
  function rsDebounce(key, fn) {
    clearTimeout(rsTypeTimers[key]);
    rsTypeTimers[key] = setTimeout(fn, 600);
  }

  function pgbSnap(id) {
    var st = pgbState[id];
    return st ? st.results.slice() : [];
  }
  function pgbLoad(id, results, total) {
    if (Array.isArray(results) && results.length === total) pgbState[id] = { results: results.slice(), streak: 0 };
    else pgbInit(id, total);
  }
  ...
  function runTask(key) {
    ...
    if (key === 'learn') {
      ...
      renderLevelSubmenu();
      // Vao thang lo trinh bai hoc (hoc sinh thay buoc tiep theo dang duoc to do)
  ...
  // O dang bai -> nhung phan luu "dang lam do" thuoc ve o do
  var HUB_RESUME_PREFIX = {
    warmup: ['warmup'], vocab: ['vocab:q:'], flash: ['flash'], grammar: ['grammar:q:'], dialog: ['dialog'],
    listen: ['listen:q:', 'listen:dialog', 'listen:wb'], game: ['game:match', 'game:fill', 'game:sort', 'game:errfix', 'game:mc'],
    speak: ['speak:ai', 'speak:rec:'], translate: ['translate:q:'], workbook: ['workbook']
  };
  ...
  function showLessonHub(levelId, lesson) {
    ...
    if (pathEnabled(lesson)) {
      grid.hidden = true;
      renderLessonPath(levelId, lesson);
      $('#lessonHub').scrollIntoView({ behavior: 'smooth', block: 'start' });
      return;
    }
    grid.hidden = false;
    $('#lessonPath').hidden = true;
    $('.hub-progress-label').textContent = 'Tiến độ tổng';
  ...
  /* ---------------- Lo trinh bai hoc 7 buoc (dang thu: HSK 2 (3.0) Bai 1) ----------------
     Moi bai la 1 con duong co thu tu (引 dan nhap → 懂 hieu → 会 luyen → 用 van dung →
     验 kiem tra) thay cho luoi o ngang hang. Buoc sau khong bi khoa, chi duoc goi y.
     Thanh "Tiep tuc →" (#pathBar) hien o duoi man hinh khi dang lam 1 buoc. */

  // Ap dung cho moi bai cua moi cap co trang bai hoc (HSK 1-4, HSK 3.0, YCT)
  function pathEnabled(lesson) { return !!(lesson && /\/lessons\/[\w-]*bai-\d+\.html$/.test(lesson.fullPageUrl || '')); }

  var PATH_STEPS = [
    { key: 'warmup', title: 'Khởi động', stage: '引', desc: 'Nhìn tranh, đoán nghĩa từ mới', min: 2, icon: '🎯', section: 'warmupPractice',
      open: function (l, s) { showWarmupPractice(l, s); } },
    { key: 'vocab', title: 'Học từ', stage: '懂', desc: 'Thẻ từ có tranh, âm thanh và âm Hán Việt', min: 5, icon: '📚', section: 'vocabPractice',
      open: function (l, s) { showVocabPractice(l, s); } },
    { key: 'dialog', title: 'Hội thoại', stage: '懂', desc: 'Nghe → đọc theo → đóng vai', min: 5, icon: '💬', section: 'dialoguePractice',
      open: function (l, s) { showDialoguePractice(l, s); } },
    { key: 'grammar', title: 'Ngữ pháp', stage: '会', desc: 'Xem ví dụ, rút ra công thức, luyện ngay', min: 5, icon: '📐', section: 'grammarPractice',
      open: function (l, s) { showGrammarPractice(l, s); } },
    { key: 'game', title: 'Luyện tập', stage: '会', desc: 'Nối câu, điền từ, sắp xếp câu', min: 5, icon: '🧩', section: 'gamePractice',
  ...
  // Buoc ma bai khong co noi dung (vd HSK 1 (3.0) bai 1 la bai mo dau, khong co ngu phap)
  var pathSkipped = {};
  function pathStepSkipped(lesson, step) { return !!pathSkipped[lesson.fullPageUrl + '|' + step.key]; }

  function pathStepDone(lesson, step) {
    if (pathStepSkipped(lesson, step)) return true;
    if (step.key === 'final') {
      var f = getLessonScores(lesson).final;
      return !!(f && f.total > 0);
    }
  ...
  function renderLessonPath(levelId, lesson) {
    ...
    // Bai khong co ngu phap: danh dau bo qua roi ve lai (chi kiem tra 1 lan / bai)
    ...
    list.innerHTML = PATH_STEPS.map(function (s, i) {
      ...
      return '<li class="lp-step ' + state + '">' +
        '<button type="button" class="lp-btn" data-path-step="' + i + '">' +
          ...
          '</span>' +
          '<span class="lp-go" aria-hidden="true">' + (i === cur ? 'Học ngay →' : '→') + '</span>' +
        '</button></li>';
    }).join('');
    $all('[data-path-step]', list).forEach(function (btn) {
      btn.addEventListener('click', function () { openPathStep(levelId, lesson, parseInt(btn.getAttribute('data-path-step'), 10)); });
    });
  ...
  var pathState = null; // { levelId, lesson, idx }
  function openPathStep(levelId, lesson, idx) {
  ...
  // Thanh duoi man hinh: "Buoc 3/7 · Hoi thoai" + nut "Tiep tuc →"
  function updatePathBar() {
    ...
    bar.innerHTML =
      ...
      '<div class="pb-actions">' +
        '<button type="button" class="pb-map" id="pbMap">Lộ trình</button>' +
        '<button type="button" class="pb-next" id="pbNext">' + (next
          ? '<span class="pb-long">Tiếp tục: ' + next.title + ' →</span><span class="pb-short">Tiếp tục →</span>'
          : 'Xem kết quả →') + '</button>' +
      '</div>';
    bar.hidden = false;
    document.body.classList.add('has-path-bar');
    ...
    $('#pbMap').addEventListener('click', function () {
      ...
      pathState = null;
      updatePathBar();
      if (st) showLessonHub(st.levelId, st.lesson);
    });
  }

  // An/hien thanh khi hoc sinh chuyen man hinh (nut quay lai, menu...)
  (function watchPathBar() {
  ...
  /* ---------------- Kiem tra cuoi bai: 10 cau tron → diem, sao, chung nhan ---------------- */

  var fqState = null;
  function showFinalPractice(levelId, lesson) {
    currentHubLevelId = levelId;
    currentHubLesson = lesson;
  ...
  function fqRender() {
    ...
    if (st.pos >= total) { fqFinish(); return; }
    var it = st.items[st.pos];
    ...
    $all('.vp-option-btn', wrap).forEach(function (btn) {
      btn.addEventListener('click', function () {
        ...
        $all('.vp-option-btn', wrap).forEach(function (b, j) {
          b.disabled = true;
          if (j === it.ans) b.classList.add('is-correct');
          else if (j === i) b.classList.add('is-wrong');
        });
        if (ok) st.score++;
        pgbRecord('fq', st.pos, ok);
        setTimeout(function () { st.pos++; fqRender(); }, 950);
      });
    });
  }
  ...
  function fqFinish() {
    ...
    var d = new Date();
    var dateTxt = String(d.getDate()).padStart(2, '0') + '/' + String(d.getMonth() + 1).padStart(2, '0') + '/' + d.getFullYear();
    var lvl = PRACTICE_LEVEL_LABEL[st.levelId] || st.levelId;
    wrap.innerHTML =
      '<div class="fq-result">' +
        '<div class="fq-score">' + st.score + '<span>/' + total + '</span></div>' +
        '<p class="fq-msg">' + (st.score === total ? 'Xuất sắc! Đúng tất cả 🎉' : pass ? 'Rất tốt! Bạn đã nắm được bài này.' : 'Cần ôn thêm một chút — làm lại để nhận chứng nhận nhé!') + '</p>' +
        '<p class="fq-stars">Mỗi câu đúng lần đầu được +1 ⭐</p>' +
      '</div>' +
      (pass
        ? '<div class="cert" id="fqCert">' +
            '<div class="cert-seal">喜汉</div>' +
            '<p class="cert-eyebrow">Chứng nhận hoàn thành</p>' +
            '<p class="cert-name">' + idiomEsc(name) + '</p>' +
            '<p class="cert-text">đã hoàn thành <b>Bài ' + st.lesson.number + ': ' + idiomEsc(st.lesson.titleHanzi || '') + '</b><br>' + idiomEsc(lvl) + ' · đạt <b>' + st.score + '/' + total + '</b> điểm</p>' +
            '<div class="cert-foot"><span>Ngày ' + dateTxt + '</span><span>Cô Hoàng Thùy · Hi Hán 喜汉</span></div>' +
          '</div>'
        : '') +
      classCtaHtml() +
      '<div class="fq-actions">' +
        ...
        '<button type="button" class="btn btn-primary" id="fqDone">Xem kết quả cả bài →</button>' +
      '</div>';
    $('#fqRetry').addEventListener('click', function () { showFinalPractice(st.levelId, st.lesson); });
    if (pass) $('#fqPrint').addEventListener('click', function () {
      document.body.classList.add('print-cert');
      window.print();
      setTimeout(function () { document.body.classList.remove('print-cert'); }, 500);
    });
    $('#fqDone').addEventListener('click', function () { pathState = null; updatePathBar(); showResultsPractice(st.levelId, st.lesson); });
  }
  ...
  function showWarmupPractice(levelId, lesson) {
    ...
    loadLessonWarmup(lesson).then(function (data) {
      ...
      var saved = wpItems.length ? rsGet('warmup') : null;
      var letters = wpItems.map(function (w) { return w.letter; }).sort().join('|');
      if (saved && saved.cards && saved.cards.slice().sort().join('|') === letters &&
          saved.opts && saved.opts.slice().sort().join('|') === letters && saved.done && saved.done.length) {
        renderWarmupPractice(saved);
        rsNotice('#wpContent', 'Đã khôi phục bài làm dở: ghép đúng <b>' + saved.done.length + '/' + wpItems.length + '</b> cặp.', function () {
          rsClear('warmup');
          showWarmupPractice(levelId, lesson);
        });
      } else {
  ...
  var wpOrder = { cards: [], opts: [] };

  function wpByLetters(letters) {
    return letters.map(function (l) { return wpItems.filter(function (w) { return w.letter === l; })[0]; });
  }

  function wpSave() {
    var done = wpItems.filter(function (w) { return wpDone.has(w.letter); }).map(function (w) { return w.letter; });
    if (!done.length || done.length === wpItems.length) { rsClear('warmup'); return; }
    rsSet('warmup', { cards: wpOrder.cards, opts: wpOrder.opts, done: done });
  }

  function renderWarmupPractice(saved) {
    var wrap = $('#wpContent');
    if (!wpItems.length) {
      wrap.innerHTML = '<p style="color:var(--color-gray-500);">Bài học này chưa có phần khởi động.</p>';
      return;
    }
    var shuffledCards = saved ? wpByLetters(saved.cards) : shuffle(wpItems.slice());
    var shuffledOpts = saved ? wpByLetters(saved.opts) : shuffle(wpItems.slice());
    wpOrder = {
      cards: shuffledCards.map(function (w) { return w.letter; }),
      opts: shuffledOpts.map(function (w) { return w.letter; })
    };
    wrap.innerHTML =
    ...
    if (saved) {
      rsQuiet(function () {
        saved.done.forEach(function (letter) {
          var card = document.getElementById('wpc_' + letter);
          if (!card || wpDone.has(letter)) return;
          wpSelectCard(letter, card);
          wpSelectOpt(letter);
        });
      });
      $('#wpFb').innerHTML = '';
  ...
  // ---- Ghi nho bai lam trong sach bai tap: chup lai trang thai trang dang mo ----
  var wbSaved = { tab: 0, secs: {} };
  var wbRestoreOrders = null;
  var wbResumeBound = false;

  function wbSnapshot() {
    var wrap = $('#wbContent');
    return {
      inputs: $all('.wb-drill-input', wrap).map(function (inp) { return inp.value; }),
      revealed: $all('.wb-block[id^="wbdrill-"]', wrap).map(function (b, i) { return b.classList.contains('is-revealed') ? i : -1; }).filter(function (i) { return i >= 0; }),
      tone: $all('.wb-tonemc-item', wrap).map(function (item) { return item.hasAttribute('data-picked') ? parseInt(item.getAttribute('data-picked'), 10) : -1; }),
      match: $all('.wb-pymatch-wrap', wrap).map(function (w) {
        var done = wbMatchDoneMap[w.getAttribute('data-wb-match-id')];
        return {
          order: $all('.mg-item[data-wpy-ri]', w).map(function (el) { return parseInt(el.getAttribute('data-wpy-ri'), 10); }),
          done: done ? Array.from(done) : []
        };
      }),
      mt: $('#mtSubmitBtn', wrap) ? { answers: mtAnswers, submitted: mtSubmitted } : null
    };
  }

  function wbSnapHasWork(snap) {
    if (!snap) return false;
    return (snap.inputs || []).some(function (v) { return v; }) || (snap.revealed || []).length ||
      (snap.tone || []).some(function (v) { return v >= 0; }) ||
      (snap.match || []).some(function (m) { return m.done && m.done.length; }) ||
      !!(snap.mt && Object.keys(snap.mt.answers || {}).length);
  }

  function wbPersist() {
    var any = Object.keys(wbSaved.secs).some(function (k) { return wbSnapHasWork(wbSaved.secs[k]); });
    if (!any && !wbSaved.tab) rsClear('workbook');
    else rsSet('workbook', wbSaved);
  }

  function wbSaveCurrent() {
    if (rsRestoring || !wbSections.length || !$('#wbContent .wb-block, #wbContent .wb-audio-box')) return;
    var snap = wbSnapshot();
    if (wbSnapHasWork(snap)) wbSaved.secs[wbActiveIdx] = snap;
    else delete wbSaved.secs[wbActiveIdx];
    wbPersist();
  }

  function wbBindResume() {
    if (wbResumeBound) return;
    wbResumeBound = true;
    var wrap = $('#wbContent');
    wrap.addEventListener('click', function () { setTimeout(wbSaveCurrent, 0); });
    wrap.addEventListener('input', function (e) {
      if (e.target.classList.contains('wb-drill-input')) rsDebounce('workbook', wbSaveCurrent);
    });
  }

  function wbApplySnapshot(snap) {
    var wrap = $('#wbContent');
    rsQuiet(function () {
      var inputs = $all('.wb-drill-input', wrap);
      (snap.inputs || []).forEach(function (v, i) { if (inputs[i]) inputs[i].value = v || ''; });
      var drills = $all('.wb-block[id^="wbdrill-"]', wrap);
      (snap.revealed || []).forEach(function (i) {
        var btn = drills[i] && $('.wb-reveal-btn', drills[i]);
        if (btn) btn.click();
      });
      var items = $all('.wb-tonemc-item', wrap);
      (snap.tone || []).forEach(function (oi, i) {
        var btn = oi >= 0 && items[i] && $all('.wb-tonemc-opt', items[i])[oi];
        if (btn) btn.click();
      });
      $all('.wb-pymatch-wrap', wrap).forEach(function (w, k) {
        var m = (snap.match || [])[k];
        if (!m) return;
        (m.done || []).forEach(function (li) {
          var left = $('.mg-item[data-wpy-li="' + li + '"]', w);
          var right = $('.mg-item[data-wpy-ri="' + li + '"]', w);
          if (left && right) { left.click(); right.click(); }
        });
      });
      if (snap.mt) {
        Object.keys(snap.mt.answers || {}).forEach(function (qKey) {
          var btn = $('.mt-opt[data-mt-q="' + qKey + '"][data-mt-key="' + snap.mt.answers[qKey] + '"]', wrap);
          if (btn) btn.click();
        });
        if (snap.mt.submitted && $('#mtSubmitBtn')) $('#mtSubmitBtn').click();
      }
    });
  }
  ...
  function wbRenderPyMatch(block) {
    ...
    var savedOrder = wbRestoreOrders && wbRestoreOrders.shift();
    if (savedOrder && savedOrder.length === block.items.length &&
  ...
  function renderWbContent() {
    ...
    var snap = wbSaved.secs && wbSaved.secs[wbActiveIdx];
    wbRestoreOrders = snap && snap.match ? snap.match.map(function (m) { return m.order; }) : null;
    ...
    if (mockTestBlock) {
      ...
      if (submitBtn) {
        submitBtn.addEventListener('click', function () {
          if (mtSubmitted) {
            // "Lam lai" de kiem tra: bo phan bai kiem tra da luu, giu cac phan khac
            if (wbSaved.secs[wbActiveIdx]) wbSaved.secs[wbActiveIdx].mt = null;
            mtAnswers = {};
            mtSubmitted = false;
            renderWbContent();
            wbSaveCurrent();
    ...
    $all('.wb-tonemc-opt', wrap).forEach(function (btn) {
      btn.addEventListener('click', function () {
        ...
        item.setAttribute('data-picked', $all('.wb-tonemc-opt', item).indexOf(btn));
  ...
  function showVocabPractice(levelId, lesson) {
    ...
    rsHideNotice('#vpContent');
    loadLessonVocab(lesson).then(function (vocabData) {
      vpVocab = vocabData;
      var savedMode = rsGet('vocab:mode');
      vpMode = VP_TABS.some(function (t) { return t.id === savedMode; }) ? savedMode : 'list';
      vpQuiz = null;
  ...
  function renderVpTabs() {
    ...
    VP_TABS.forEach(function (t) {
      ...
      btn.addEventListener('click', function () {
        ...
        vpQuiz = null;
        rsHideNotice('#vpContent');
        rsSet('vocab:mode', t.id === 'list' ? null : t.id);
        renderVpTabs();
        renderVpContent();
      });
      wrap.appendChild(btn);
  ...
  function renderVpQuiz(mode) {
    var rsKey = 'vocab:q:' + mode;
    if (!vpQuiz) {
      var saved = rsGet(rsKey);
      if (saved && Array.isArray(saved.order) && saved.order.length === vpVocab.length && saved.pos > 0 && saved.pos < saved.order.length) {
        vpQuiz = { order: saved.order.slice(), pos: saved.pos, score: saved.score || 0 };
        pgbLoad('vpq', saved.results, vpQuiz.order.length);
        rsNotice('#vpContent', 'Tiếp tục từ câu <b>' + (saved.pos + 1) + '/' + saved.order.length + '</b> · đúng ' + (saved.score || 0) + ' câu.', function () {
          rsClear(rsKey);
          vpQuiz = null;
          renderVpQuiz(mode);
        });
    ...
    if (vpQuiz.pos >= vpQuiz.order.length) {
      ...
      $('#vpRestart').addEventListener('click', function () { rsClear(rsKey); vpQuiz = null; renderVpQuiz(mode); });
      return;
    }
    ...
    $all('.vp-option-btn', wrap).forEach(function (btn, i) {
      btn.addEventListener('click', function () {
        ...
        rsSet(rsKey, { order: vpQuiz.order, pos: vpQuiz.pos + 1, score: vpQuiz.score, results: pgbSnap('vpq') });
        setTimeout(function () {
          vpQuiz.pos++;
          renderVpQuiz(mode);
        }, 700);
  ...
  function showFlashcardPractice(levelId, lesson) {
    ...
    rsHideNotice('#fcContent');
    loadLessonVocab(lesson).then(function (vocabData) {
      fcVocab = vocabData;
      fcIndex = 0;
      fcTally = { yes: 0, no: 0 };
      var saved = rsGet('flash');
      if (saved && saved.total === vocabData.length && saved.index > 0 && saved.index < vocabData.length) {
        fcIndex = saved.index;
        fcTally = { yes: saved.yes || 0, no: saved.no || 0 };
        rsNotice('#fcContent', 'Tiếp tục từ thẻ <b>' + (fcIndex + 1) + '/' + vocabData.length + '</b> · đã nhớ ' + fcTally.yes + ' từ.', function () {
          rsClear('flash');
          fcIndex = 0;
          fcTally = { yes: 0, no: 0 };
          renderFlashcard();
  ...
  function renderFlashcard() {
    ...
    $('#fcContent').innerHTML =
      ...
      '<div class="fc-card-wrap"><div class="fc-card" id="fcCard">' +
        ...
        '<div class="fc-face fc-back">' +
          ...
          (word.hv ? '<span class="fc-hv">Hán Việt: <b>' + word.hv + '</b></span>' : '') +
    ...
    function fcSave() { rsSet('flash', { index: fcIndex, yes: fcTally.yes, no: fcTally.no, total: total }); }
  ...
  var GRAMMAR_EXERCISES = {
    ...
    '/lessons/hsk4-bai-19.html': [
      { point: 'Đại từ nghi vấn phiếm chỉ 「谁 / 什么 / 怎么……都」',
        items: [
          { context: 'Hoàn thành câu: không cần xin lỗi, ai cũng có lúc cẩu thả điền sai.',
            pre: '没关系，不用道歉，', blank: '谁', post: '都有粗心填错的时候。', options: ['谁', '哪儿', '怎么'], answer: 0,
            explanation: '谁都 — bất kỳ ai; ôn lại 粗心 (Bài 15).' },
          { context: 'Hoàn thành câu: sáng dậy nghĩ thế nào cũng không nhớ ra.',
            pre: '昨天我做了一个奇怪的梦，但是早上起来', blank: '怎么', post: '想都想不起来。', options: ['怎么', '什么', '谁'], answer: 0,
            explanation: '怎么想都想不起来 — bất kỳ cách nào; ôn lại 想起来 (Bài 15), 梦 (Bài 17).' },
          { context: 'Hoàn thành câu: thứ gì cũng phải qua nỗ lực mới có được.',
            pre: '"世界上没有免费的午餐"这句话是说，', blank: '什么', post: '东西都要通过努力才能得到。', options: ['什么', '哪', '谁'], answer: 0,
            explanation: '什么东西都 — bất kỳ thứ gì; ôn lại 免费 (Bài 5), 通过 (Bài 9).' },
          { type: 'judge', context: 'Xét câu sau có đúng ngữ pháp không.',
            sentence: '每个人都有自己的生活，谁也不能替别人做决定。', isCorrect: true,
            explanation: 'Câu đúng — bài luyện tập SGK (1): 谁也不 — không ai; ôn lại 替 (Bài 15), 决定 (Bài 5).' },
          { type: 'judge', context: 'Xét câu sau có đúng ngữ pháp không.',
            sentence: '他什么爱吃，所以很瘦。', isCorrect: false, correctVersion: '他什么都不爱吃，所以很瘦。',
            explanation: 'Phiếm chỉ cần 都/也 phối hợp: 什么都不爱吃. Ôn lại 瘦 (Bài 7).' },
          { type: 'sort', context: 'Sắp xếp câu: anh ấy làm gì cũng thấy không có ý nghĩa, nên luôn không vui.',
            words: ['他', '做', '什么', '都', '觉得', '没有', '意思', '，', '所以', '他', '总是', '不', '开心', '。'], answer: '他做什么都觉得没有意思，所以他总是不开心。',
            explanation: '什么都 — bài luyện tập SGK (2); ôn lại 开心 (Bài 13).' },
          { type: 'sort', context: 'Sắp xếp câu: tôi đã luyện mấy lần rồi, sao vẫn làm thế nào cũng không chuẩn?',
            words: ['我', '已经', '练习', '了', '好几', '遍', '了', '，', '怎么', '还是', '怎么', '做', '都', '不', '标准', '？'], answer: '我已经练习了好几遍了，怎么还是怎么做都不标准？',
            explanation: '怎么做都 — bài luyện tập SGK (3); ôn lại 遍 (Bài 13), 标准 (Bài 10).' },
          { type: 'translate', context: 'Dịch câu sau, dùng đại từ phiếm chỉ.',
            promptLang: 'vi', prompt: 'Múa là một loại "ngôn ngữ", người nước nào cũng có thể hiểu được.',
            answer: '舞蹈是一种"语言"，哪个国家的人都能看懂。', answerPy: 'Wǔdǎo shì yì zhǒng "yǔyán", nǎ ge guójiā de rén dōu néng kàndǒng.',
            explanation: '哪个……都 — bất kỳ nước nào; 舞蹈 từ mới.' },
          { type: 'translate', context: 'Dịch câu sau.',
            promptLang: 'vi', prompt: 'Cuối tuần đi đâu cũng đông người, chúng ta cứ ở nhà nghỉ đi.',
            answer: '周末去哪儿都人很多，咱们还是在家休息吧。', answerPy: 'Zhōumò qù nǎr dōu rén hěn duō, zánmen háishi zài jiā xiūxi ba.',
            explanation: '哪儿都 — bất kỳ đâu; ôn lại 还是……吧 (Bài 1).' },
        ] },
      { point: '「上」(bổ ngữ)',
        items: [
          { context: 'Hoàn thành câu: xem ra hôm nay không ăn được sủi cảo thịt dê rồi.',
            pre: '看来今天吃不', blank: '上', post: '羊肉饺子了。', options: ['上', '到', '了'], answer: 0,
            explanation: '吃不上 — không đạt được mục đích ăn; ôn lại 看来 (Bài 8).' },
          { context: 'Hoàn thành câu: giờ tắc đường dữ thế, xem ra không kịp chuyến bay 8 giờ.',
            pre: '现在堵车这么厉害，看来今天坐不', blank: '上', post: '8点那趟去上海的飞机了。', options: ['上', '下', '起'], answer: 0,
            explanation: '坐不上 — không kịp lên (chuyến bay); ôn lại 厉害 (Bài 11), 趟 (Bài 17).' },
          { context: 'Hoàn thành câu: vừa nghe anh trai thi đỗ tiến sĩ, cả nhà rất vui.',
            pre: '一听到哥哥考', blank: '上', post: '了经济专业的博士，我们全家都特别高兴。', options: ['上', '完', '好'], answer: 0,
            explanation: '考上 — thi đỗ (đạt mục đích); ôn lại 博士 (Bài 16), 经济 (Bài 10).' },
          { type: 'judge', context: 'Xét câu sau có đúng ngữ pháp không.',
            sentence: '要是你想去云南旅游应该早点儿买火车票，否则可能买不上票。', isCorrect: true,
            explanation: 'Câu đúng — bài luyện tập SGK (1): 买不上; ôn lại 要是 (Bài 5), 否则 (Bài 11).' },
          { type: 'judge', context: 'Xét câu sau có đúng ngữ pháp không.',
            sentence: '早就听说这个饭馆儿的菜特别好吃，今天终于吃上不了。', isCorrect: false, correctVersion: '早就听说这个饭馆儿的菜特别好吃，今天终于吃上了。',
            explanation: 'Bài luyện tập SGK (2): khẳng định 吃上了; phủ định phải là 吃不上 (不 chèn giữa), không đặt 不 sau 上. Ôn lại 终于 (Bài 8).' },
          { type: 'sort', context: 'Sắp xếp câu: cuối tuần chúng tôi vốn định đi xem phim, nhưng vé bán hết rồi, không xem được.',
            words: ['周末', '我们', '本来', '想', '去', '看', '电影', '，', '可是', '票', '卖完', '了', '，', '没', '看上', '。'], answer: '周末我们本来想去看电影，可是票卖完了，没看上。',
            explanation: '没看上 — bài luyện tập SGK (3); ôn lại 本来 (Bài 10).' },
          { type: 'sort', context: 'Sắp xếp câu: chỉ cần kiên trì, bạn nhất định có thể thi đỗ trường đại học lý tưởng.',
            words: ['只要', '坚持', '，', '你', '一定', '能', '考上', '理想', '的', '大学', '。'], answer: '只要坚持，你一定能考上理想的大学。',
            explanation: '考上; ôn lại 只要 (Bài 8), 坚持/理想 (Bài 9).' },
          { type: 'translate', context: 'Dịch câu sau, dùng bổ ngữ 上.',
            promptLang: 'vi', prompt: 'Trận cầu lông đông người quá, đi muộn e là không có chỗ chơi.',
            answer: '羽毛球场人太多，去晚了恐怕打不上。', answerPy: 'Yǔmáoqiú chǎng rén tài duō, qù wǎnle kǒngpà dǎbushàng.',
            explanation: '打不上 + 羽毛球/场 (từ mới) + 恐怕 (Bài 16).' },
          { type: 'translate', context: 'Dịch câu sau.',
            promptLang: 'vi', prompt: 'Anh ấy nỗ lực nhiều năm, cuối cùng đã mua được nhà ở Bắc Kinh.',
            answer: '他努力了很多年，终于在北京买上房子了。', answerPy: 'Tā nǔlìle hěn duō nián, zhōngyú zài Běijīng mǎishàng fángzi le.',
            explanation: '买上 — đạt mục đích; ôn lại 终于 (Bài 8).' },
        ] },
      { point: '「出来」so sánh với 「起来」',
        items: [
          { context: 'Hoàn thành câu: có người trong lòng có ý kiến gì, miệng nói thẳng ra ngay.',
            pre: '有的人心里有什么看法或意见，嘴上就会直接说', blank: '出来', post: '。', options: ['出来', '起来', '下来'], answer: 0,
            explanation: '说出来 — từ ẩn thành lộ; ôn lại 看法 (Bài 15), 直接 (Bài 12).' },
          { context: 'Hoàn thành câu: vừa rồi thấy bạn múa, không nhìn ra bạn múa hay thế!',
            pre: '刚才我在旁边看到你跳中国舞了，没看', blank: '出来', post: '你跳得这么好！', options: ['出来', '起来', '过来'], answer: 0,
            explanation: '看出来 — nhận ra (từ ẩn thành hiện).' },
          { context: 'Hoàn thành câu: âm thanh nghe rất quen, nhưng tôi nhất thời không nhớ ra.',
            pre: '声音听着挺熟悉的，不过我一下子想不', blank: '起来', post: '了。', options: ['起来', '出来', '过去'], answer: 0,
            explanation: 'Bài 做一做 SGK (3): thông tin vốn có nhưng quên → 想不起来 (Bài 15), không phải 想不出来; ôn lại 熟悉 (Bài 9).' },
          { type: 'judge', context: 'Xét câu sau có đúng ngữ pháp không.',
            sentence: '这个主意好像是三班的一个学生想出来的。', isCorrect: true,
            explanation: 'Câu đúng — bài 做一做 SGK (2): 想出来 = nghĩ ra ý mới; ôn lại 主意 (Bài 6).' },
          { type: 'judge', context: 'Xét câu sau có đúng ngữ pháp không.',
            sentence: '久坐办公室的人有时间一定要站出来活动活动。', isCorrect: false, correctVersion: '久坐办公室的人有时间一定要站起来活动活动。',
            explanation: 'Bài 做一做 SGK (1): đứng dậy = hướng từ dưới lên → 站起来, không dùng 出来.' },
          { type: 'sort', context: 'Sắp xếp câu: giám đốc bảo Tiểu Lý viết một bài văn, anh ấy chỉ một ngày đã viết ra.',
            words: ['经理', '让', '小李', '写', '一篇', '文章', '，', '他', '只', '用', '了', '一天', '就', '写', '出来', '了', '。'], answer: '经理让小李写一篇文章，他只用了一天就写出来了。',
            explanation: '写出来 — bài luyện tập SGK (1); ôn lại 文章 (Bài 11).' },
          { type: 'sort', context: 'Sắp xếp câu: gặp chuyện phiền não, bạn nên nghĩ cách để mình thoát ra khỏi tâm trạng không vui.',
            words: ['遇到', '烦恼', '事', '时', '，', '你', '应该', '想', '一些', '办法', '让', '自己', '从', '不高兴', '的', '心情', '中', '走', '出来', '。'], answer: '遇到烦恼事时，你应该想一些办法让自己从不高兴的心情中走出来。',
            explanation: 'Bài 做一做 SGK (4): 走出来 — ra khỏi; ôn lại 烦恼 (Bài 8), 心情 (Bài 8).' },
          { type: 'translate', context: 'Dịch câu sau, dùng 出来.',
            promptLang: 'vi', prompt: 'Hôm nay thầy hỏi một câu, cả lớp không ai trả lời được.',
            answer: '今天老师问了个问题，全班没有人回答出来。', answerPy: 'Jīntiān lǎoshī wènle ge wèntí, quán bān méiyǒu rén huídá chūlái.',
            explanation: 'Bài luyện tập SGK (2): 回答出来 — nói ra được đáp án.' },
          { type: 'translate', context: 'Dịch câu sau, phân biệt 出来 / 起来.',
            promptLang: 'vi', prompt: 'Có người tuy thấy khuyết điểm của người khác, nhưng không nói ra; anh ấy nhớ lại lời thầy dạy: nên nói ra một cách lịch sự.',
            answer: '有的人虽然看到了别人的缺点，但是不说出来；他想起来老师说过的话：应该礼貌地说出来。', answerPy: 'Yǒude rén suīrán kàndàole biérén de quēdiǎn, dànshì bù shuō chūlái; tā xiǎng qǐlai lǎoshī shuōguo de huà: yīnggāi lǐmào de shuō chūlái.',
            explanation: 'Bài luyện tập SGK (3): 说出来 (lộ ra) + 想起来 (nhớ lại, Bài 15); ôn lại 缺点 (Bài 8), 礼貌 (Bài 16).' },
        ] },
      { point: '「总的来说」',
        items: [
          { context: 'Hoàn thành câu: nhìn chung, căn nhà này anh rất hài lòng.',
            pre: '广告上说房子交通方便，周围很安静，厨房很大。', blank: '总的来说', post: '，这套房子他很满意。', options: ['总的来说', '一般来说', '拿……来说'], answer: 0,
            explanation: '总的来说 — nhận xét tổng thể sau chi tiết; ôn lại 交通 (Bài 18), 满意 (Bài 6).' },
          { context: 'Hoàn thành câu: lương tuy không cao nhưng thưởng nhiều, nên nhìn chung thu nhập khá.',
            pre: '这个公司的工资虽然不算很高，但是奖金很多，所以', blank: '总的来说', post: '收入还不错。', options: ['总的来说', '除此以外', '于是'], answer: 0,
            explanation: '总的来说 — tổng kết; ôn lại 工资/奖金/收入 (Bài 10).' },
          { context: 'Hoàn thành câu: nhìn chung, cảnh Lệ Giang đẹp, người nhiệt tình, đi đó là lựa chọn tốt.',
            pre: '很多人问去丽江旅游怎么样，', blank: '总的来说', post: '，丽江景色不错，那里的人也很热情。', options: ['总的来说', '对我来说', '到底'], answer: 0,
            explanation: '总的来说 — đánh giá chung; ôn lại 景色 (Bài 6), 热情 (Bài 3).' },
          { type: 'judge', context: 'Xét câu sau có đúng ngữ pháp không.',
            sentence: '去国外留学有好处也有坏处，总的来说，好处比坏处多。', isCorrect: true,
            explanation: 'Câu đúng — bài luyện tập SGK (1); ôn lại 好处 (Bài 6), 坏处.' },
          { type: 'judge', context: 'Xét câu sau có đúng ngữ pháp không.',
            sentence: '总的来说，我今天早上吃了一个包子。', isCorrect: false, correctVersion: '我今天早上吃了一个包子。',
            explanation: '总的来说 dùng để tổng kết nhiều tình huống/chi tiết, không dùng cho một sự việc đơn lẻ, cụ thể.' },
          { type: 'sort', context: 'Sắp xếp câu: mỗi người tuy đều gặp khó khăn, nhưng nhìn chung cuộc sống vẫn tốt đẹp.',
            words: ['每个', '人', '虽然', '都', '会', '遇到', '一些', '困难', '，', '总的来说', '，', '生活', '仍然', '是', '美好', '的', '。'], answer: '每个人虽然都会遇到一些困难，总的来说，生活仍然是美好的。',
            explanation: 'Bài luyện tập SGK (2); ôn lại 仍然 (Bài 17), 美好.' },
          { type: 'sort', context: 'Sắp xếp câu: bộ quần áo này giá hơi cao, nhưng chất lượng tốt, kiểu dáng cũng đẹp, nhìn chung đáng mua.',
            words: ['这件', '衣服', '价格', '有点儿', '高', '，', '但', '质量', '好', '，', '样子', '也', '好看', '，', '总的来说', '值得', '买', '。'], answer: '这件衣服价格有点儿高，但质量好，样子也好看，总的来说值得买。',
            explanation: 'Bài luyện tập SGK (3); ôn lại 质量 (Bài 6), 值得 (Bài 7).' },
          { type: 'translate', context: 'Dịch câu sau, dùng 总的来说.',
            promptLang: 'vi', prompt: 'Nhà thuê tuy hơi ồn, nhưng giao thông tiện, bếp rộng, nhìn chung tôi khá hài lòng.',
            answer: '租的房子虽然有点儿吵，但是交通方便，厨房很大，总的来说我挺满意的。', answerPy: 'Zū de fángzi suīrán yǒudiǎnr chǎo, dànshì jiāotōng fāngbiàn, chúfáng hěn dà, zǒng de lái shuō wǒ tǐng mǎnyì de.',
            explanation: '总的来说 + 租/吵/厨房 (từ mới) + 挺……的 (Bài 6).' },
          { type: 'translate', context: 'Dịch câu sau.',
            promptLang: 'vi', prompt: 'Học kỳ này bài nhiều, thi cũng khó, nhưng nhìn chung tôi học được rất nhiều.',
            answer: '这个学期课很多，考试也很难，但总的来说我学到了很多东西。', answerPy: 'Zhège xuéqī kè hěn duō, kǎoshì yě hěn nán, dàn zǒng de lái shuō wǒ xuédàole hěn duō dōngxi.',
            explanation: '总的来说 + 学期 (từ mới).' },
        ] },
      { point: '「在于」',
        items: [
          { context: 'Hoàn thành câu: người ta thường nói "sự sống nằm ở vận động".',
            pre: '人们常说"生命', blank: '在于', post: '运动"，所以很多人一到周末就会到体育馆打几场球。', options: ['在于', '由于', '对于'], answer: 0,
            explanation: '在于 — nằm ở, chính là; ôn lại 生命 (Bài 8), 由于 (Bài 10), 对于 (Bài 12).' },
          { context: 'Hoàn thành câu: then chốt chọn nghề nằm ở hứng thú.',
            pre: '选择职业的关键', blank: '在于', post: '兴趣，当你喜欢做一件事情的时候，你会带着热情去工作。', options: ['在于', '在', '是否'], answer: 0,
            explanation: '关键在于 + danh từ; ôn lại 职业/关键 (Bài 10).' },
          { context: 'Hoàn thành câu: khác biệt nằm ở chỗ có người chấp nhận thất bại và tiếp tục nỗ lực.',
            pre: '区别', blank: '在于', post: '有的人能接受失败，找到失败的原因并继续努力。', options: ['在于', '关于', '至于'], answer: 0,
            explanation: '区别在于 + mệnh đề; ôn lại 区别 (Bài 8), 接受 (Bài 6), 失败 (Bài 9).' },
          { type: 'judge', context: 'Xét câu sau có đúng ngữ pháp không.',
            sentence: '每个人对幸福的理解不同，有人认为幸福在于有钱，有人认为幸福在于健康。', isCorrect: true,
            explanation: 'Câu đúng — bài luyện tập SGK (1); ôn lại 幸福 (Bài 10), 理解 (Bài 8).' },
          { type: 'judge', context: 'Xét câu sau có đúng ngữ pháp không.',
            sentence: '成功的关键在于。', isCorrect: false, correctVersion: '成功的关键在于坚持。',
            explanation: '在于 BẮT BUỘC phải có tân ngữ (danh từ/động từ/câu ngắn) theo sau. Ôn lại 坚持 (Bài 9).' },
          { type: 'sort', context: 'Sắp xếp câu: người đạt thành công đều trải qua nhiều thất bại, khác biệt nằm ở chỗ họ không từ bỏ.',
            words: ['取得', '成功', '的', '人', '都', '经历', '过', '许多', '失败', '，', '区别', '在于', '他们', '没有', '放弃', '。'], answer: '取得成功的人都经历过许多失败，区别在于他们没有放弃。',
            explanation: 'Bài luyện tập SGK (2); ôn lại 取得 (Bài 9), 经历 (Bài 9), 放弃 (Bài 9).' },
          { type: 'sort', context: 'Sắp xếp câu: anh ấy làm việc thường đạt hiệu quả gấp đôi, then chốt nằm ở phương pháp đúng.',
            words: ['他', '做', '事情', '常常', '能', '达到', '事半功倍', '的', '效果', '，', '关键', '在于', '方法', '正确', '。'], answer: '他做事情常常能达到事半功倍的效果，关键在于方法正确。',
            explanation: 'Bài luyện tập SGK (3); ôn lại 达到/事半功倍 (Bài 12), 效果 (Bài 9).' },
          { type: 'translate', context: 'Dịch câu sau, dùng 在于.',
            promptLang: 'vi', prompt: 'Thuê được nhà tốt hay không, then chốt nằm ở chỗ chủ nhà có thân thiện hay không.',
            answer: '能不能租到好房子，关键在于房东是否友好。', answerPy: 'Néng bu néng zūdào hǎo fángzi, guānjiàn zàiyú fángdōng shìfǒu yǒuhǎo.',
            explanation: '在于 + mệnh đề với 是否 (Bài 18); 租/房东 từ mới; 友好 (Bài 12).' },
          { type: 'translate', context: 'Dịch câu sau.',
            promptLang: 'vi', prompt: 'Học ngoại ngữ có giỏi hay không, không nằm ở việc thông minh, mà nằm ở việc kiên trì luyện tập mỗi ngày.',
            answer: '外语学得好不好，不在于聪明，而在于每天坚持练习。', answerPy: 'Wàiyǔ xué de hǎo bu hǎo, bú zàiyú cōngming, ér zàiyú měi tiān jiānchí liànxí.',
            explanation: '不在于……而在于……; ôn lại 坚持 (Bài 9), 而 (Bài 8).' },
        ] },
    ],
    '/lessons/hsk4-bai-20.html': [
      { point: '「Động từ + 着 + Động từ + 着」',
        items: [
          { context: 'Hoàn thành câu: được, vậy tôi yên tâm, đừng đang lái thì hết xăng.',
            pre: '好，那我就放心了，别', blank: '开着开着', post: '没油了。', options: ['开着开着', '开了开了', '开来开去'], answer: 0,
            explanation: 'V着V着 — đang làm thì xảy ra việc khác; ôn lại 放心 (Bài 5).' },
          { context: 'Hoàn thành câu: cô ấy kể một lúc thì tự cười trước.',
            pre: '她', blank: '讲着讲着', post: '自己就先笑了，而大家却不明白她到底为什么笑。', options: ['讲着讲着', '讲了讲', '一讲'], answer: 0,
            explanation: '讲着讲着就……; ôn lại 却 (Bài 2), 到底 (Bài 16).' },
          { context: 'Hoàn thành câu: có lúc nằm một lúc thì ngủ thiếp đi.',
            pre: '有时候', blank: '躺着躺着', post: '就安静地睡着了。', options: ['躺着躺着', '躺一躺', '躺下'], answer: 0,
            explanation: '躺着躺着就睡着了; ôn lại 躺 (Bài 7), 睡着 (Bài 4).' },
          { type: 'judge', context: 'Xét câu sau có đúng ngữ pháp không.',
            sentence: '小时候的记忆总是那么美好，想着想着我就笑了。', isCorrect: true,
            explanation: 'Câu đúng — bài luyện tập SGK (1): 想着想着; ôn lại 记忆 (Bài 15).' },
          { type: 'judge', context: 'Xét câu sau có đúng ngữ pháp không.',
            sentence: '那天的羊肉饺子太好吃了，我吃着着吃就吃了三十个。', isCorrect: false, correctVersion: '那天的羊肉饺子太好吃了，我吃着吃着就吃了三十个。',
            explanation: 'Cấu trúc đúng: V着V着 (吃着吃着), không phải 吃着着吃 — bài luyện tập SGK (2); ôn lại 饺子 (Bài 19).' },
          { type: 'sort', context: 'Sắp xếp câu: cuốn tiểu thuyết này viết chán quá, tôi đọc một lúc thì ngủ mất.',
            words: ['这篇', '小说', '写', '得', '太', '无聊', '了', '，', '我', '看着', '看着', '就', '睡着', '了', '。'], answer: '这篇小说写得太无聊了，我看着看着就睡着了。',
            explanation: 'Bài luyện tập SGK (3); ôn lại 无聊 (Bài 4), 小说 (Bài 2).' },
          { type: 'sort', context: 'Sắp xếp câu: hai người bạn cũ nói chuyện một lúc thì nhắc đến những chuyện thời đại học.',
            words: ['两个', '老朋友', '聊着', '聊着', '就', '说起', '了', '大学', '时候', '的', '事情', '。'], answer: '两个老朋友聊着聊着就说起了大学时候的事情。',
            explanation: '聊着聊着 + 说起 (ngữ pháp bài).' },
          { type: 'translate', context: 'Dịch câu sau, dùng V着V着.',
            promptLang: 'vi', prompt: 'Anh ấy đang đi thì đột nhiên nhớ ra chìa khóa để quên ở nhà.',
            answer: '他走着走着突然想起来钥匙忘在家里了。', answerPy: 'Tā zǒuzhe zǒuzhe tūrán xiǎng qǐlai yàoshi wàng zài jiāli le.',
            explanation: '走着走着 + 想起来 (Bài 15) + 钥匙 (từ mới).' },
          { type: 'translate', context: 'Dịch câu sau.',
            promptLang: 'vi', prompt: 'Cô ấy vừa xem ảnh vừa kể, kể một lúc thì khóc.',
            answer: '她一边看照片一边讲，讲着讲着就哭了。', answerPy: 'Tā yìbiān kàn zhàopiàn yìbiān jiǎng, jiǎngzhe jiǎngzhe jiù kū le.',
            explanation: '一边……一边 (Bài 2) + 讲着讲着.' },
        ] },
      { point: '「一……就……」',
        items: [
          { context: 'Hoàn thành câu: đợi con gái vừa về là tôi báo tin vui này.',
            pre: '等女儿一回来我', blank: '就', post: '告诉她这个好消息。', options: ['就', '才', '再'], answer: 0,
            explanation: '一……就 — nối tiếp ngay, chủ ngữ khác nhau; ôn lại 消息 (Bài 9).' },
          { context: 'Hoàn thành câu: 100 năm trước, nhựa vừa xuất hiện đã được đón nhận rộng rãi.',
            pre: '100年前，塑料', blank: '一', post: '出现就受到人们的普遍欢迎。', options: ['一', '刚', '才'], answer: 0,
            explanation: '一出现就受到; ôn lại 塑料 (Bài 14), 普遍 (Bài 13).' },
          { context: 'Hoàn thành câu: con trai hồi nhỏ hễ nói là đỏ mặt.',
            pre: '儿子小时候一说话', blank: '就', post: '脸红，回答老师问题的时候声音也很小。', options: ['就', '却', '也'], answer: 0,
            explanation: '一……就 nghĩa điều kiện "hễ … là"; ôn lại 脸 (Bài 8).' },
          { type: 'judge', context: 'Xét câu sau có đúng ngữ pháp không.',
            sentence: '明天我要去上海出差，一到就给你打电话。', isCorrect: true,
            explanation: 'Câu đúng — bài luyện tập SGK (1); ôn lại 出差 (Bài 14).' },
          { type: 'judge', context: 'Xét câu sau có đúng ngữ pháp không.',
            sentence: '妈妈进房间一，他就把手机装了起来。', isCorrect: false, correctVersion: '妈妈一进房间，他就把手机装了起来。',
            explanation: '一 đứng TRƯỚC động từ 1 (一进房间), 就 trước động từ 2 — SGK ví dụ (5).' },
          { type: 'sort', context: 'Sắp xếp câu: dì Trương vẫn không thích ứng khí hậu miền Bắc, hễ đến mùa đông là bị cảm.',
            words: ['张阿姨', '还是', '不', '适应', '北方', '的', '气候', '，', '一', '到', '冬天', '就', '感冒', '。'], answer: '张阿姨还是不适应北方的气候，一到冬天就感冒。',
            explanation: 'Bài luyện tập SGK (2); ôn lại 适应 (Bài 8), 气候 (Bài 9).' },
          { type: 'sort', context: 'Sắp xếp câu: giám đốc Mã sáng đi họp, họp vừa xong là về ngay.',
            words: ['马经理', '上午', '去', '开会', '了', '，', '会', '一', '结束', '就', '回来', '。'], answer: '马经理上午去开会了，会一结束就回来。',
            explanation: 'Bài luyện tập SGK (3); ôn lại 结束 (Bài 7).' },
          { type: 'translate', context: 'Dịch câu sau, dùng 一……就…….',
            promptLang: 'vi', prompt: 'Tôi không ăn cay được, vừa ăn cay là ho.',
            answer: '我不能吃辣，一吃辣的东西就咳嗽。', answerPy: 'Wǒ bù néng chī là, yì chī là de dōngxi jiù késou.',
            explanation: '一……就 điều kiện + 辣 (từ mới) + 咳嗽 (Bài 7).' },
          { type: 'translate', context: 'Dịch câu sau.',
            promptLang: 'vi', prompt: 'Cha mẹ phải để con biết từ nhỏ, không phải mọi thứ cứ khóc là có được.',
            answer: '父母必须让孩子从小就知道，不是所有的东西一哭就能得到。', answerPy: 'Fùmǔ bìxū ràng háizi cóng xiǎo jiù zhīdào, bú shì suǒyǒu de dōngxi yì kū jiù néng dédào.',
            explanation: 'SGK ví dụ (4); ôn lại 所有 (Bài 8), 得到 (Bài 6).' },
        ] },
      { point: '「究竟」so sánh với 「到底」',
        items: [
          { context: 'Hoàn thành câu: rốt cuộc mùa nào đi Lệ Giang du lịch thì tốt hơn?',
            pre: '', blank: '究竟', post: '哪个季节去丽江旅游比较好呢？', options: ['究竟', '到底看', '难道'], answer: 0,
            explanation: '究竟 đứng trước đại từ nghi vấn làm chủ ngữ; ôn lại 季节 (Bài 7).' },
          { context: 'Hoàn thành câu: cô ấy quyết tâm nhất định giảm cân đến cùng.',
            pre: '她决心一定要将减肥进行', blank: '到底', post: '。', options: ['到底', '究竟', '终于'], answer: 0,
            explanation: 'Bài 做一做 SGK (2): 进行到底 — động từ "đến cùng" → chỉ 到底; ôn lại 减肥 (Bài 7), 进行 (Bài 13).' },
          { context: 'Hoàn thành câu: sự sống rốt cuộc từ đâu đến?',
            pre: '有些问题我们仍然无法回答，例如，生命', blank: '究竟', post: '从哪里来？', options: ['究竟', '仍然', '竟然'], answer: 0,
            explanation: '究竟 truy vấn (到底 cũng được); ôn lại 仍然 (Bài 17), 无法 (Bài 12), 例如 (Bài 10).' },
          { type: 'judge', context: 'Xét câu sau có đúng ngữ pháp không.',
            sentence: '这场比赛太精彩了，你猜究竟谁能先进球？', isCorrect: true,
            explanation: 'Câu đúng — bài 做一做 SGK (1): 究竟/到底 đều được; ôn lại 精彩 (Bài 11), 猜 (Bài 7), 场 (Bài 19).' },
          { type: 'judge', context: 'Xét câu sau có đúng ngữ pháp không.',
            sentence: '无论做什么事情都不会很容易就成功，只有坚持究竟，才会有希望。', isCorrect: false, correctVersion: '无论做什么事情都不会很容易就成功，只有坚持到底，才会有希望。',
            explanation: 'Bài 做一做 SGK (4): 坚持到底 — 到底 làm động từ; 究竟 không có nghĩa này. Ôn lại 坚持 (Bài 9).' },
          { type: 'sort', context: 'Sắp xếp câu: bài này tôi làm ba lần, mỗi lần ra đáp án khác nhau, lạ thật, rốt cuộc sai ở đâu?',
            words: ['这个', '题', '我', '做', '了', '三', '遍', '，', '可', '每次', '算', '出来', '的', '答案', '都', '不', '一样', '，', '真', '奇怪', '，', '究竟', '是', '哪儿', '出', '问题', '了', '？'], answer: '这个题我做了三遍，可每次算出来的答案都不一样，真奇怪，究竟是哪儿出问题了？',
            explanation: 'Bài 做一做 SGK (3); ôn lại 遍 (Bài 13), 算出来 (Bài 19), 奇怪 (Bài 5).' },
          { type: 'sort', context: 'Sắp xếp câu: giận dữ phần nhiều do hiểu lầm gây ra, vì vậy khi thấy mình sắp giận, tốt nhất làm rõ rốt cuộc là chuyện gì.',
            words: ['生气', '多', '是', '由', '误会', '引起', '的', '，', '因此', '当', '你', '觉得', '自己', '要', '生气', '的', '时候', '，', '最好', '先', '弄', '清楚', '究竟', '是', '怎么', '回事', '。'], answer: '生气多是由误会引起的，因此当你觉得自己要生气的时候，最好先弄清楚究竟是怎么回事。',
            explanation: 'Bài 做一做 SGK (5); ôn lại 由 (Bài 13), 误会/引起 (Bài 12), 弄清楚 (Bài 15).' },
          { type: 'translate', context: 'Dịch câu sau, dùng 究竟.',
            promptLang: 'vi', prompt: 'Tôi muốn thử cách trang web nói, xem rốt cuộc có hiệu quả không.',
            answer: '我想试试网站上说的这个方法，看看究竟有没有效果。', answerPy: 'Wǒ xiǎng shìshi wǎngzhàn shang shuō de zhège fāngfǎ, kànkan jiūjìng yǒu méiyǒu xiàoguǒ.',
            explanation: 'Bài luyện tập SGK (2); ôn lại 网站 (Bài 18), 效果 (Bài 9).' },
          { type: 'translate', context: 'Dịch câu sau, phân biệt 究竟 / 到底.',
            promptLang: 'vi', prompt: 'Dạo này mỗi ngày khách đến cửa hàng không quá 20 người, rốt cuộc là vì sao? Chúng ta phải điều tra đến cùng.',
            answer: '最近每天来店里的顾客不超过20个，究竟是为什么？我们得调查到底。', answerPy: 'Zuìjìn měi tiān lái diàn li de gùkè bù chāoguò èrshí ge, jiūjìng shì wèishénme? Wǒmen děi diàochá dàodǐ.',
            explanation: 'Bài luyện tập SGK (3): 究竟 (phó từ) + 调查到底 (động từ); ôn lại 顾客 (Bài 6), 超过 (Bài 2), 调查 (Bài 6).' },
        ] },
      { point: '「起来」(nghĩa mở rộng)',
        items: [
          { context: 'Hoàn thành câu: tiếng Thượng Hải nghe như ngoại ngữ.',
            pre: '你跟上海人对话时，会发现上海话听', blank: '起来', post: '就像外语一样。', options: ['起来', '出来', '过来'], answer: 0,
            explanation: '听起来 — đánh giá từ phương diện nghe; ôn lại 像……一样 (Bài 6).' },
          { context: 'Hoàn thành câu: bạn nghe, cậu ấy lại hát lên rồi.',
            pre: '邻居小王最近一定遇到了很多开心的事，你听，他又唱', blank: '起来', post: '了。', options: ['起来', '出来', '下去'], answer: 0,
            explanation: '唱起来 — bắt đầu và tiếp diễn; ôn lại 邻居 (Bài 7), 开心 (Bài 13).' },
          { context: 'Hoàn thành câu: dù giờ mới tháng 4, thời tiết đã bắt đầu nóng lên.',
            pre: '尽管现在才4月，天气已经开始慢慢热', blank: '起来', post: '。', options: ['起来', '上来', '出来'], answer: 0,
            explanation: '热起来 — bắt đầu; ôn lại 尽管 (Bài 9).' },
          { type: 'judge', context: 'Xét câu sau có đúng ngữ pháp không.',
            sentence: '我本来以为做生意很简单，没想到做起来这么难。', isCorrect: true,
            explanation: 'Câu đúng — bài luyện tập SGK (1): 做起来 — đánh giá; ôn lại 本来 (Bài 10), 生意 (Bài 12).' },
          { type: 'judge', context: 'Xét câu sau có đúng ngữ pháp không.',
            sentence: '他的汉语语法虽然有点儿小错误，但是说起来出很流利。', isCorrect: false, correctVersion: '他的汉语语法虽然有点儿小错误，但是说起来很流利。',
            explanation: 'Bài luyện tập SGK (3): 说起来 + tính từ đánh giá, không thêm 出; ôn lại 语法 (Bài 3), 流利 (Bài 2).' },
          { type: 'sort', context: 'Sắp xếp câu: công viên ngày nào cũng có nhiều người già hát Kinh kịch, họ vừa hát lên là hàng xóm đều đến xem.',
            words: ['公园', '里', '每天', '都', '有', '很多', '老人', '唱', '京剧', '，', '他们', '一', '唱', '起来', '，', '邻居', '们', '都', '来', '看', '。'], answer: '公园里每天都有很多老人唱京剧，他们一唱起来，邻居们都来看。',
            explanation: 'Bài luyện tập SGK (2): 唱起来 + 一……就 (ngữ pháp bài); ôn lại 京剧 (Bài 13).' },
          { type: 'sort', context: 'Sắp xếp câu: bạn nhấc đồ trên bàn lên trước, tôi lau xong rồi bạn đặt xuống.',
            words: ['你', '先', '把', '桌子', '上', '的', '东西', '拿', '起来', '，', '我', '擦', '完', '之后', '你', '再', '放', '下来', '。'], answer: '你先把桌子上的东西拿起来，我擦完之后你再放下来。',
            explanation: 'SGK ví dụ (1): 拿起来 (hướng lên) vs 放下来; ôn lại 擦 (Bài 11).' },
          { type: 'translate', context: 'Dịch câu sau, dùng 起来.',
            promptLang: 'vi', prompt: 'Món Hồ Nam trông rất cay, nhưng ăn vào lại rất thơm.',
            answer: '湖南菜看起来很辣，但是吃起来却很香。', answerPy: 'Húnán cài kàn qilai hěn là, dànshì chī qilai què hěn xiāng.',
            explanation: '看起来/吃起来 (đánh giá) + 辣/香 (từ mới) + 却 (Bài 2).' },
          { type: 'translate', context: 'Dịch câu sau.',
            promptLang: 'vi', prompt: 'Vừa nghe tin vui, con gái lập tức vui lên, còn cười lên nữa.',
            answer: '一听到好消息，女儿马上高兴起来，还笑了起来。', answerPy: 'Yì tīngdào hǎo xiāoxi, nǚ\'ér mǎshàng gāoxìng qǐlai, hái xiàole qǐlai.',
            explanation: '高兴起来/笑起来 (bắt đầu) + 一……就 (ngữ pháp bài).' },
        ] },
      { point: '「Động từ + 起」',
        items: [
          { context: 'Hoàn thành câu: nói đến đồ ăn, ấn tượng sâu nhất với tôi là món Hồ Nam.',
            pre: '说', blank: '起', post: '吃的东西，给我印象最深的是湖南菜。', options: ['起', '出', '到'], answer: 0,
            explanation: '说起 + danh từ — nói đến; ôn lại 印象 (Bài 6).' },
          { context: 'Hoàn thành câu: nói đến trận tennis ấy, hai người hào hứng không dừng lại được.',
            pre: '聊', blank: '起', post: '那场网球比赛，他们俩就兴奋得停不下来了。', options: ['起', '完', '着'], answer: 0,
            explanation: '聊起 — bàn về; ôn lại 场 (Bài 19), 兴奋 (Bài 6), 停不下来.' },
          { context: 'Hoàn thành câu: nếu có ai hỏi đến chuyện đó, bạn cứ nói chưa nhận được thông báo.',
            pre: '要是有人问', blank: '起', post: '那件事，你就告诉他你还没接到通知。', options: ['起', '过', '好'], answer: 0,
            explanation: '问起 — hỏi đến; ôn lại 要是……就 (Bài 5), 通知 (Bài 4).' },
          { type: 'judge', context: 'Xét câu sau có đúng ngữ pháp không.',
            sentence: '一提起大学时候的事，大家总是有说不完的话。', isCorrect: true,
            explanation: 'Câu đúng — bài luyện tập SGK (1): 提起; ôn lại 提 (Bài 8), 说不完.' },
          { type: 'judge', context: 'Xét câu sau có đúng ngữ pháp không.',
            sentence: '看着这些可爱的孩子，我想起了自己的童年吃起。', isCorrect: false, correctVersion: '看着这些可爱的孩子，我想起了自己的童年。',
            explanation: 'V+起 chỉ dùng với động từ như 说/谈/讲/问/提/聊/回忆/想 (想起 Bài 15) — 吃起 không đúng; bài luyện tập SGK (2). Ôn lại 可爱 (Bài 15).' },
          { type: 'sort', context: 'Sắp xếp câu: dù đã qua hơn mười năm, nhắc đến chuyện đó anh ấy vẫn rất xúc động.',
            words: ['尽管', '已经', '过去', '十多', '年', '了', '，', '回忆', '起', '那件', '事', '他', '仍然', '很', '激动', '。'], answer: '尽管已经过去十多年了，回忆起那件事他仍然很激动。',
            explanation: 'Bài luyện tập SGK (3): 回忆起; ôn lại 尽管 (Bài 9), 回忆 (Bài 8), 仍然 (Bài 17), 激动 (Bài 16).' },
          { type: 'sort', context: 'Sắp xếp câu: nói đến du lịch, ấn tượng sâu nhất với tôi là chuyến Lệ Giang năm ngoái.',
            words: ['谈', '起', '旅行', '，', '给', '我', '印象', '最', '深', '的', '是', '去年', '去', '丽江', '的', '那', '一趟', '。'], answer: '谈起旅行，给我印象最深的是去年去丽江的那一趟。',
            explanation: '谈起 + 趟 (Bài 17); 旅行 từ mới.' },
          { type: 'translate', context: 'Dịch câu sau, dùng V+起.',
            promptLang: 'vi', prompt: 'Nhắc đến hướng dẫn viên đó, ai cũng nhớ những truyện cười cô ấy kể.',
            answer: '提起那个导游，谁都记得她讲的笑话。', answerPy: 'Tíqǐ nàge dǎoyóu, shéi dōu jìde tā jiǎng de xiàohua.',
            explanation: '提起 + 导游 (Bài 16) + 谁都 (Bài 19) + 笑话 (từ mới).' },
          { type: 'translate', context: 'Dịch câu sau.',
            promptLang: 'vi', prompt: 'Nói đến món ăn miền Nam, nhiều người miền Bắc đều thích canh ở đó.',
            answer: '说起南方菜，很多北方人都喜欢那里的汤。', answerPy: 'Shuōqǐ nánfāng cài, hěn duō běifāng rén dōu xǐhuan nàli de tāng.',
            explanation: '说起 + 汤 (từ mới).' },
        ] },
    ]
  ...
  function renderGrammarExercise() {
    ...
    var grKey = 'grammar:q:' + grMode;
    if (!grQuiz) {
      var grSaved = rsGet(grKey);
      if (grSaved && grSaved.total === items.length && grSaved.pos > 0 && grSaved.pos < items.length) {
        grQuiz = { pos: grSaved.pos, score: grSaved.score || 0 };
        pgbLoad('grq', grSaved.results, items.length);
        rsNotice('#grContent', 'Tiếp tục bài tập ngữ pháp từ câu <b>' + (grSaved.pos + 1) + '/' + items.length + '</b> · đúng ' + (grSaved.score || 0) + ' câu.', function () {
          rsClear(grKey);
          grQuiz = null;
          renderGrammarExercise();
        });
      } else {
        grQuiz = { pos: 0, score: 0 };
        pgbInit('grq', items.length);
      }
    }
    ...
    if (grQuiz.pos >= items.length) {
      ...
      $('#grRestart').addEventListener('click', function () { rsClear(grKey); grQuiz = null; renderGrammarExercise(); });
      return;
    }
  ...
  function grFinishQuestion(isCorrect, explanation) {
    ...
    rsSet('grammar:q:' + grMode, { pos: grQuiz.pos + 1, score: grQuiz.score, total: grCurrentItems().length, results: pgbSnap('grq') });
  ...
  function showDialoguePractice(levelId, lesson) {
    ...
    loadLessonDialog(lesson).then(function (dialogData) {
      ...
      var dpSaved = rsGet('dialog');
      if (dpSaved && dpSaved.scenes === dialogData.length && dpSaved.quizTotal === dpTotalQuiz) {
        dpIndex = Math.min(Math.max(0, dpSaved.index || 0), dialogData.length - 1);
        dpQuizState = dpSaved.quiz || {};
        (dpSaved.viewed || []).forEach(function (i) { dpViewed.add(i); });
        if (dpTotalQuiz > 0) pgbLoad('dpq', dpSaved.results, dpTotalQuiz);
        rsNotice('#dpContent', 'Tiếp tục ở <b>đoạn hội thoại ' + (dpIndex + 1) + '/' + dialogData.length + '</b> như lần trước.', function () {
          rsClear('dialog');
          showDialoguePractice(levelId, lesson);
        });
      }
  ...
  // Luu doan dang xem + dap an nghe hieu; xem het cac doan va tra loi het thi coi nhu xong
  function dpSave() {
    var allQuizDone = dpScenes.every(function (sc, i) {
      var st = dpQuizState[i];
      return !(sc.preQuiz && sc.preQuiz.length) || (st && st.revealed);
    });
    if (dpViewed && dpViewed.size === dpScenes.length && allQuizDone) { rsClear('dialog'); return; }
    var hasWork = dpIndex > 0 || (dpViewed && dpViewed.size > 1) || Object.keys(dpQuizState).some(function (k) {
      return Object.keys((dpQuizState[k] && dpQuizState[k].answers) || {}).length;
    });
    if (!hasWork) return;
    rsSet('dialog', {
      index: dpIndex, scenes: dpScenes.length, quizTotal: dpTotalQuiz, quiz: dpQuizState,
      viewed: dpViewed ? Array.from(dpViewed) : [], results: dpTotalQuiz > 0 ? pgbSnap('dpq') : []
    });
  }
  ...
  function showListenPractice(levelId, lesson) {
    ...
    var savedLpMode = rsGet('listen:mode');
    lpMode = ['meaning', 'hanzi', 'dialog'].indexOf(savedLpMode) >= 0 ? savedLpMode : 'meaning';
    $all('#lpTabs .vp-tab').forEach(function (t) { t.classList.toggle('active', t.getAttribute('data-lp-tab') === lpMode); });
    rsHideNotice('#lpContent');
    lpBindResume();
  ...
  // ---- Ghi nho bai nghe lam do (de nghe sach bai tap + nghe hoi thoai) ----
  var lpResumeBound = false;
  function lpBindResume() {
    if (lpResumeBound) return;
    lpResumeBound = true;
    var wrap = $('#lpContent');
    wrap.addEventListener('click', function (e) {
      if (e.target.closest('.lp-dict-reveal, .lp-judge-btn, .lp-mcdict-reveal, .lp-mc-opt, .lp-opt-btn')) setTimeout(lpSaveCurrent, 0);
    });
    wrap.addEventListener('input', function (e) {
      if (e.target.classList.contains('lp-dict-input')) rsDebounce('listen', lpSaveCurrent);
    });
  }

  function lpPickedIndex(btns) {
    var wrong = -1, right = -1;
    btns.forEach(function (b, i) {
      if (b.classList.contains('is-wrong')) wrong = i;
      if (b.classList.contains('is-correct')) right = i;
    });
    return wrong >= 0 ? wrong : right;
  }

  function lpSaveCurrent() {
    if (rsRestoring || !currentHubLesson) return;
    var wrap = $('#lpContent');
    if (lpListenData && lpListenData.type === 'workbook') {
      if (!lpWorkbookScore) return;
      var inputs = {};
      $all('.lp-dict-input', wrap).forEach(function (inp) { if (inp.value) inputs[inp.id] = inp.value; });
      var judge = {};
      $all('.lp-judge.done', wrap).forEach(function (box) {
        var chosen = $('.lp-judge-btn.chosen', box);
        if (chosen) judge[box.getAttribute('data-judge-num')] = chosen.getAttribute('data-judge-val');
      });
      var mcdict = $all('.lp-mcdict-reveal', wrap).filter(function (b) { return b.disabled; }).map(function (b) { return b.getAttribute('data-mcdict-reveal'); });
      var mc = {};
      Object.keys(lpWorkbookScore.mcCorrect).forEach(function (num) {
        mc[num] = lpPickedIndex($all('.lp-mc-opt[data-mc-num="' + num + '"]', wrap));
      });
      var dict = Object.keys(lpWorkbookScore.dictCorrect);
      var total = lpWbTotal();
      if (dict.length + Object.keys(mc).length >= total) { rsClear('listen:wb'); return; }
      if (!dict.length && !Object.keys(mc).length && !Object.keys(inputs).length && !Object.keys(judge).length) { rsClear('listen:wb'); return; }
      rsSet('listen:wb', { total: total, inputs: inputs, dict: dict, judge: judge, mcdict: mcdict, mc: mc, results: pgbSnap('lpwq') });
    } else if (lpMode === 'dialog') {
      var answers = {};
      var totalQ = 0;
      $all('.lp-question-opts', wrap).forEach(function (group) {
        totalQ++;
        var btns = $all('.lp-opt-btn', group);
        if (!btns.length || !btns[0].disabled) return;
        answers[btns[0].getAttribute('data-gi') + '_' + btns[0].getAttribute('data-qi')] = lpPickedIndex(btns);
      });
      var n = Object.keys(answers).length;
      if (!n || n >= totalQ) rsClear('listen:dialog');
      else rsSet('listen:dialog', { total: totalQ, answers: answers });
    }
  }
  ...
  function renderListenWorkbook() {
    ...
    var wbSavedListen = rsGet('listen:wb');
    if (wbSavedListen && wbSavedListen.total === total && !Object.keys(lpWorkbookScore.dictCorrect).length && !Object.keys(lpWorkbookScore.mcCorrect).length) {
      rsQuiet(function () {
        Object.keys(wbSavedListen.inputs || {}).forEach(function (id) {
          var inp = document.getElementById(id);
          if (inp && wrap.contains(inp)) inp.value = wbSavedListen.inputs[id];
        });
        (wbSavedListen.dict || []).forEach(function (num) {
          var b = $('.lp-dict-reveal[data-reveal-num="' + num + '"]', wrap);
          if (b) b.click();
        });
        Object.keys(wbSavedListen.judge || {}).forEach(function (num) {
          var b = $('.lp-judge[data-judge-num="' + num + '"] .lp-judge-btn[data-judge-val="' + wbSavedListen.judge[num] + '"]', wrap);
          if (b) b.click();
        });
        (wbSavedListen.mcdict || []).forEach(function (num) {
          var b = $('.lp-mcdict-reveal[data-mcdict-reveal="' + num + '"]', wrap);
          if (b) b.click();
        });
        Object.keys(wbSavedListen.mc || {}).forEach(function (num) {
          var b = $all('.lp-mc-opt[data-mc-num="' + num + '"]', wrap)[wbSavedListen.mc[num]];
          if (b) b.click();
        });
      });
      var doneCount = (wbSavedListen.dict || []).length + Object.keys(wbSavedListen.mc || {}).length;
      if (doneCount || Object.keys(wbSavedListen.inputs || {}).length) {
        rsNotice('#lpContent', 'Đã khôi phục đề nghe làm dở: xong <b>' + doneCount + '/' + total + '</b> câu.', function () {
          rsClear('listen:wb');
          lpWorkbookScore = null;
          renderListenWorkbook();
        });
      }
    }
  }
  ...
  function renderListenMCQuiz() {
    ...
    var lpKey = 'listen:q:' + lpMode;
    if (!lpQuiz) {
      var lpSaved = rsGet(lpKey);
      if (lpSaved && Array.isArray(lpSaved.order) && lpSaved.order.length === lpVocab.length && lpSaved.pos > 0 && lpSaved.pos < lpSaved.order.length) {
        lpQuiz = { order: lpSaved.order.slice(), pos: lpSaved.pos, score: lpSaved.score || 0 };
        rsNotice('#lpContent', 'Tiếp tục luyện nghe từ câu <b>' + (lpSaved.pos + 1) + '/' + lpSaved.order.length + '</b> · đúng ' + (lpSaved.score || 0) + ' câu.', function () {
          rsClear(lpKey);
          lpQuiz = null;
          renderListenMCQuiz();
        });
      } else {
        lpQuiz = { order: shuffle(lpVocab.map(function (_, i) { return i; })), pos: 0, score: 0 };
      }
    }
    ...
    if (lpQuiz.pos >= total) {
      ...
      $('#lpRestart').addEventListener('click', function () { rsClear(lpKey); lpQuiz = null; renderListenMCQuiz(); });
      return;
    }
    ...
    $all('.vp-option-btn', wrap).forEach(function (btn, i) {
      btn.addEventListener('click', function () {
        ...
        rsSet(lpKey, { order: lpQuiz.order, pos: lpQuiz.pos + 1, score: lpQuiz.score });
        setTimeout(function () {
          lpQuiz.pos++;
          renderListenMCQuiz();
        }, 900);
  ...
  function renderListenDialogPassages() {
    ...
    var dlgSaved = rsGet('listen:dialog');
    var dlgTotal = $all('.lp-question-opts', wrap).length;
    if (dlgSaved && dlgSaved.total === dlgTotal && Object.keys(dlgSaved.answers || {}).length) {
      rsQuiet(function () {
        Object.keys(dlgSaved.answers).forEach(function (k) {
          var parts = k.split('_');
          var b = $('.lp-opt-btn[data-gi="' + parts[0] + '"][data-qi="' + parts[1] + '"][data-oi="' + dlgSaved.answers[k] + '"]', wrap);
          if (b) b.click();
        });
      });
      rsNotice('#lpContent', 'Đã khôi phục bài nghe hội thoại: trả lời <b>' + Object.keys(dlgSaved.answers).length + '/' + dlgTotal + '</b> câu.', function () {
        rsClear('listen:dialog');
        renderListenDialogPassages();
      });
    }
  }
  ...
  function showSpeakPractice(levelId, lesson) {
    ...
    rsHideNotice('#spContent');
    loadLessonSpeaking(lesson).then(function (speakingData) {
      spData = speakingData;
      var savedTier = rsGet('speak:tier');
      spTierMode = [2, 3].indexOf(savedTier) >= 0 ? savedTier : 1;
      spRecState = {};
      renderSpeakTabs();
      renderSpeakContent();
    }).catch(function () {
  ...
  function renderSpeakTabs() {
    ...
    $all('[data-sp-tier]', wrap).forEach(function (btn) {
      btn.addEventListener('click', function () {
        ...
        rsHideNotice('#spContent');
        rsSet('speak:tier', spTierMode === 1 ? null : spTierMode);
  ...
  function renderSpeakAI(wrap, data) {
    ...
    // Diem cac cau da chấm o lan hoc truoc
    var aiSaved = rsGet('speak:ai');
    if (aiSaved && aiSaved.n === spAiTasks.length && aiSaved.scores) {
      var restored = 0;
      rsQuiet(function () {
        Object.keys(aiSaved.scores).forEach(function (k) {
          var i = parseInt(k, 10), sc = aiSaved.scores[k];
          if (!spAiTasks[i] || !sc || typeof sc.total !== 'number') return;
          spAi[i] = { score: sc };
          if (sc.self) {
            $('#spAiResult' + i).innerHTML = '<div class="spai-self">Bạn tự chấm: <b>' + sc.total + '/100</b></div>';
          } else if (sc.parts) {
            spAiRenderResult(i, spAiTasks[i], sc);
          }
          $('#spAiStatus' + i).innerHTML = '<span class="spai-done">✓ Đã chấm ở lần học trước — bấm 🎙️ để nói lại.</span>';
          restored++;
        });
        spAiUpdateSummary();
      });
      if (restored) {
        rsNotice('#spContent', 'Đã khôi phục điểm <b>' + restored + '/' + spAiTasks.length + '</b> câu đã luyện nói.', function () {
          rsClear('speak:ai');
          renderSpeakContent();
        });
      }
  ...
  function spAiUpdateSummary() {
    ...
    if (done >= spAiTasks.length) {
      rsClear('speak:ai');
    } else if (done) {
      var scores = {};
      spAiTasks.forEach(function (t, i) {
        var st = spAi[i];
        if (!st || !st.score || typeof st.score.total !== 'number') return;
        var sc = st.score;
        scores[i] = sc.self ? { total: sc.total, self: true } : { total: sc.total, transcript: sc.transcript, chars: sc.chars, rate: sc.rate, parts: sc.parts, feedback: sc.feedback };
      });
      rsSet('speak:ai', { n: spAiTasks.length, scores: scores });
    }
  }
  ...
  function spRecKey() { return 'speak:rec:' + (spData && spData.questions ? 'q' : 't3'); }
  function spRecCount() { return $all('.sp-rec-box', $('#spContent')).length; }

  function spRecSave(i, d) {
    var key = spRecKey();
    var saved = rsGet(key) || { n: spRecCount(), results: {} };
    if (saved.n !== spRecCount()) saved = { n: spRecCount(), results: {} };
    saved.results[i] = {
      pronunciation: d.pronunciation, accuracy: d.accuracy, fluency: d.fluency,
      completeness: d.completeness, recognizedText: d.recognizedText || ''
    };
    if (Object.keys(saved.results).length >= saved.n) rsClear(key);
    else rsSet(key, saved);
  }

  function spRecRestore() {
    var key = spRecKey();
    var saved = rsGet(key);
    var n = spRecCount();
    if (!saved || saved.n !== n || !saved.results) return;
    var count = 0;
    Object.keys(saved.results).forEach(function (k) {
      var d = saved.results[k];
      var resultEl = $('#spRecResult' + k);
      ...
      resultEl.innerHTML = '<div class="sp-rec-scores">' +
        ...
        '</div>' + (d.recognizedText ? '<div class="sp-rec-recognized">Máy nghe được: "' + rsEsc(d.recognizedText) + '"</div>' : '');
      $('#spRecStatus' + k).textContent = '✓ Đã chấm ở lần học trước — ghi âm lại để luyện tiếp.';
      count++;
    });
    if (count) {
      rsNotice('#spContent', 'Đã khôi phục điểm <b>' + count + '/' + n + '</b> câu đã ghi âm.', function () {
        rsClear(key);
        renderSpeakContent();
      });
    }
  ...
  function showGamePractice(levelId, lesson) {
    ...
    gpMode = null;
    rsHideNotice('#gpContent');
    loadLessonGameData(lesson).then(function (data) {
      gpGameData = data;
      var savedGame = rsGet('game:mode');
      if (savedGame && (LEVEL_GAME_TYPES[levelId] || []).indexOf(savedGame) >= 0 && (data[savedGame] || []).length) gpMode = savedGame;
      renderGameScreen();
    }).catch(function () {
  ...
  function renderGameMatch(data, fresh) {
    ...
    var mSaved = fresh ? null : rsGet('game:match');
    if (mSaved && mSaved.n === data.length && Array.isArray(mSaved.order) && mSaved.order.length === data.length && (mSaved.done || []).length) {
      gmOrder = mSaved.order.slice();
    } else {
      mSaved = null;
    }
    ...
    $('#mgReset').addEventListener('click', function () { rsClear('game:match'); rsHideNotice('#gpContent'); renderGameMatch(data, true); });

    if (mSaved) {
      rsQuiet(function () {
        mSaved.done.forEach(function (li) {
          if (!data[li] || gmDone.has('L' + li)) return;
          gmClickLeft(li);
          gmClickRight(li, data);
        });
      });
      $('#mgFb').textContent = '';
      rsNotice('#gpContent', 'Tiếp tục trò ghép từ: đã ghép đúng <b>' + mSaved.done.length + '/' + data.length + '</b> cặp.', function () {
        rsClear('game:match');
        renderGameMatch(data, true);
      });
    }
  }
  ...
  function gmClickRight(ri, data) {
    ...
    if (isCorrect) {
      ...
      var doneLeft = Object.keys(data).map(Number).filter(function (li) { return gmDone.has('L' + li); });
      if (doneLeft.length >= data.length) rsClear('game:match');
      else rsSet('game:match', { n: data.length, order: gmOrder, done: doneLeft });
      fb.innerHTML = '<span style="color:var(--color-green-600);">✓ Đúng rồi!</span>';
  ...
  function renderGameFill(data) {
    ...
    var fSaved = rsGet('game:fill');
    if (fSaved && fSaved.n === data.length && (fSaved.vals || []).some(function (v) { return v; })) {
      fSaved.vals.forEach(function (v, i) { var inp = $('#fgInput' + i); if (inp) inp.value = v || ''; });
      rsNotice('#gpContent', 'Đã khôi phục các từ bạn điền dở — kiểm tra lại rồi bấm <b>Kiểm tra đáp án</b>.', function () {
        rsClear('game:fill');
        renderGameFill(data);
      });
    }
    $all('.fg-input', wrap).forEach(function (inp) {
      inp.addEventListener('input', function () {
        rsDebounce('game:fill', function () {
          var vals = data.map(function (_, i) { var el = $('#fgInput' + i); return el ? el.value : ''; });
          if (vals.some(function (v) { return v; })) rsSet('game:fill', { n: data.length, vals: vals });
          else rsClear('game:fill');
        });
      });
    });

    $('#fgCheck').addEventListener('click', function () {
      clearTimeout(rsTypeTimers['game:fill']);
      rsClear('game:fill');
      rsHideNotice('#gpContent');
      var ok = 0;
    ...
    $('#fgReset').addEventListener('click', function () { clearTimeout(rsTypeTimers['game:fill']); rsClear('game:fill'); rsHideNotice('#gpContent'); renderGameFill(data); });
  }

  var gsState = [];

  function gsSave(total) {
    if (rsRestoring) return;
    var anyPlaced = gsState.some(function (st) { return st.placed.length; });
    if (!anyPlaced) { rsClear('game:sort'); return; }
    rsSet('game:sort', {
      n: total,
      words: gsState.map(function (st) { return st.words; }),
      placed: gsState.map(function (st) { return st.placed.map(function (p) { return p.wi; }); })
    });
  }

  function renderGameSort(data, fresh) {
    gsState = data.map(function (s) {
      return { placed: [], words: shuffle(s.words.slice()) };
    });
    var sSaved = fresh ? null : rsGet('game:sort');
    var sameWords = function (a, b) { return a.slice().sort().join('\u0001') === b.slice().sort().join('\u0001'); };
    if (sSaved && sSaved.n === data.length && Array.isArray(sSaved.words) &&
        data.every(function (s, i) { return Array.isArray(sSaved.words[i]) && sameWords(sSaved.words[i], s.words); })) {
      gsState = data.map(function (s, i) {
        var words = sSaved.words[i].slice();
        var placed = (sSaved.placed[i] || []).filter(function (wi) { return words[wi] != null; })
          .map(function (wi) { return { wi: wi, word: words[wi] }; });
        return { placed: placed, words: words };
      });
    } else {
      sSaved = null;
    }
    ...
    if (sSaved) {
      var placedCount = gsState.filter(function (st) { return st.placed.length; }).length;
      rsNotice('#gpContent', 'Đã khôi phục bài sắp xếp câu làm dở (' + placedCount + '/' + data.length + ' câu đang xếp).', function () {
        rsClear('game:sort');
        renderGameSort(data, true);
      });
    }
    ...
    $('#sgReset').addEventListener('click', function () { rsClear('game:sort'); rsHideNotice('#gpContent'); renderGameSort(data, true); });
  }
  ...
  function renderGameMcList(data, opts) {
    ...
    var mcKey = 'game:' + (opts.gameKey || 'mc');
    ...
    $all('.vp-option-btn', wrap).forEach(function (btn) {
      btn.addEventListener('click', function () {
        ...
        updateScore();
        if (!rsRestoring) {
          if (Object.keys(answered).length >= data.length) { rsClear(mcKey); rsHideNotice('#gpContent'); }
          else {
            var picks = {};
            Object.keys(answered).forEach(function (k) {
              picks[k] = lpPickedIndex($all('.vp-option-btn[data-qi="' + k + '"]', wrap));
            });
            rsSet(mcKey, { n: data.length, answers: picks });
          }
        }
      });
    });

    var mcSaved = rsGet(mcKey);
    if (mcSaved && mcSaved.n === data.length && Object.keys(mcSaved.answers || {}).length) {
      rsQuiet(function () {
        Object.keys(mcSaved.answers).forEach(function (qi) {
          var b = $('.vp-option-btn[data-qi="' + qi + '"][data-oi="' + mcSaved.answers[qi] + '"]', wrap);
          if (b) b.click();
        });
      });
      rsNotice('#gpContent', 'Tiếp tục bài làm dở: đã trả lời <b>' + Object.keys(mcSaved.answers).length + '/' + data.length + '</b> câu.', function () {
        rsClear(mcKey);
        renderGameMcList(data, opts);
      });
    }
  }
  ...
  function showTranslatePractice(levelId, lesson) {
    ...
    tpDirection = rsGet('translate:dir') === 'zh2vi' ? 'zh2vi' : 'vi2zh';
    $all('.tp-dir-btn').forEach(function (btn) { btn.classList.toggle('active', btn.getAttribute('data-tp-dir') === tpDirection); });
    rsHideNotice('#tpContent');
  ...
  function renderTranslateContent() {
    ...
    var tpKey = 'translate:q:' + tpDirection;
    var tpSaved = rsGet(tpKey);
    if (!tpQuiz) {
      if (tpSaved && tpSaved.total === tpData.length && tpSaved.pos < tpData.length && (tpSaved.pos > 0 || tpSaved.draft)) {
        tpQuiz = { pos: tpSaved.pos, score: tpSaved.score || 0 };
        pgbLoad('tpq', tpSaved.results, tpData.length);
        rsNotice('#tpContent', 'Tiếp tục luyện dịch từ câu <b>' + (tpSaved.pos + 1) + '/' + tpData.length + '</b>.', function () {
          rsClear(tpKey);
          tpQuiz = null;
          renderTranslateContent();
        });
      } else {
        tpQuiz = { pos: 0, score: 0 };
        pgbInit('tpq', tpData.length);
      }
    }
    ...
    if (tpQuiz.pos >= tpData.length) {
      ...
      $('#tpRestart').addEventListener('click', function () { rsClear(tpKey); tpQuiz = null; renderTranslateContent(); });
      return;
    }
    ...
    // Ban dich dang go do cua dung cau nay
    var tpInput = $('#tpInput');
    if (tpSaved && tpSaved.draftPos === tpQuiz.pos && tpSaved.draft) tpInput.value = tpSaved.draft;
    tpInput.addEventListener('input', function () {
      rsDebounce(tpKey, function () {
        rsSet(tpKey, { total: total, pos: tpQuiz.pos, score: tpQuiz.score, results: pgbSnap('tpq'), draft: tpInput.value, draftPos: tpQuiz.pos });
      });
    });
    function tpSaveAnswered() {
      clearTimeout(rsTypeTimers[tpKey]);
      rsSet(tpKey, { total: total, pos: tpQuiz.pos + 1, score: tpQuiz.score, results: pgbSnap('tpq') });
    }
  ...
  // Chang dang do cua hom nay (cung bo cau hoi) — null neu khong con dung duoc
  function qrGetResume(lesson, qs) {
    var r = rsGet('quick', lesson);
    if (!r || r.day !== dateKey(new Date()) || r.n !== qs.length) return null;
    if (!(r.i > 0 && r.i < qs.length && r.hearts > 0 && r.leftMs > 1000)) return null;
    return r;
  }

  function qrSaveProgress() {
    var s = qrState;
    if (!s || s.finished) return;
    var nextI = s.locked ? s.i + 1 : s.i;
    var leftMs = Math.max(0, s.endAt - Date.now());
    if (s.hearts <= 0 || nextI >= s.qs.length || leftMs <= 1000 || nextI === 0) { if (nextI === 0) return; rsClear('quick', s.lesson); return; }
    rsSet('quick', {
      day: dateKey(new Date()), n: s.qs.length, i: nextI, hearts: s.hearts, score: s.score, combo: s.combo,
      bestCombo: s.bestCombo, correct: s.correct, answered: s.answered, leftMs: leftMs,
      wrongs: s.wrongs.map(function (q) { return s.qs.indexOf(q); })
    }, s.lesson);
  }

  function qrRenderIntro(lesson, qs) {
    var stats = qrLessonStats(lesson);
    var qrResume = qrGetResume(lesson, qs);
    var counts = [0, 0, 0, 0];
    ...
    wrap.innerHTML =
      '<div class="qr-intro">' +
        ...
        '<div class="qr-intro-body">' +
          ...
          (qrResume
            ? '<div class="qr-resume">' +
                '<p class="qr-resume-text">⏳ Chặng đang dở: đã vượt <b>' + qrResume.i + '/' + qs.length + '</b> chướng ngại · còn <b>' + Math.floor(qrResume.leftMs / 60000) + ':' + ('0' + Math.floor(qrResume.leftMs / 1000) % 60).slice(-2) + '</b> · ❤ ' + qrResume.hearts + '</p>' +
                '<button type="button" class="qr-start" id="qrContinue">Chạy tiếp chặng đang dở</button>' +
                '<button type="button" class="qr-restart-link" id="qrStart">Chơi lại từ đầu</button>' +
              '</div>'
    ...
    $('#qrStart').addEventListener('click', function () { rsClear('quick', lesson); qrStartGame(lesson, qs); });
    if (qrResume) $('#qrContinue').addEventListener('click', function () { qrStartGame(lesson, qs, qrResume); });
  }
  ...
  function qrStartGame(lesson, qs, resume) {
    qrStop();
    qrState = {
    ...
    if (resume) {
      qrState.i = resume.i;
      qrState.hearts = resume.hearts;
      qrState.score = resume.score || 0;
      qrState.combo = resume.combo || 0;
      qrState.bestCombo = resume.bestCombo || 0;
      qrState.correct = resume.correct || 0;
      qrState.answered = resume.answered || resume.i;
      qrState.wrongs = (resume.wrongs || []).map(function (wi) { return qs[wi]; }).filter(Boolean);
      qrState.endAt = Date.now() + resume.leftMs;
    }
    var wrap = $('#qrContent');
    wrap.innerHTML =
      '<div class="qr-play">' +
  ...
  function qrTick() {
    ...
    if ($('#quickReviewPractice').hidden) { qrSaveProgress(); qrStop(); return; }
    var left = Math.max(0, Math.round((qrState.endAt - Date.now()) / 1000));
    if (left % 5 === 0) qrSaveProgress();
    var el = $('#qrTimer');
    if (el) {
  ...
  function qrResumeInfo(lesson) {
    var r = rsGet('quick', lesson);
    if (!r || r.day !== dateKey(new Date()) || !(r.i > 0 && r.i < r.n && r.hearts > 0 && r.leftMs > 1000)) return '';
    return '⏳ Đang dở ' + r.i + '/' + r.n;
  }
  ...
  function qrHubEntryHtml(lesson) {
    ...
    return '<button type="button" class="qr-entry" id="qrEntry">' +
      ...
      '<span class="qr-entry-status">' + (qrResumeInfo(lesson) || (st.doneToday ? '✓ Hôm nay đã ôn' : 'Hôm nay chưa ôn')) + (st.streak ? '<em>🔥 ' + st.streak + ' ngày</em>' : '') + '</span>' +
      '</button>';
  }
  ...
  function sfxQueue(kind) {
    ...
    sfxTimer = setTimeout(function () {
      ...
      sfxPending = null;
      sfxTimer = null;
      if (k && !sfxMuted && sfxActive()) sfxPlay(k);
    }, 25);
  }
  ...
  function initAuth() {
    ...
    sidebarUserBtn.addEventListener('click', function () {
      ...
      if (auth) {
        ...
        // Bai lam do thuoc ve tai khoan nay: gui not len may chu roi xoa khoi may
        // (may dung chung — hoc sinh khac dang nhap sau khong lam tiep nham bai cua ban).
        rsFlush(true);
  ...
  function init() {
    ...
    initAnswerSounds();
    initShellNav();
    writeJSON(RESUME_KEY, rsPrune(rsStore()));
    // Dong tab / chuyen ung dung: gui ngay phan con dang cho dong bo
    document.addEventListener('visibilitychange', function () { if (document.hidden) rsFlush(true); });
    window.addEventListener('pagehide', function () { rsFlush(true); });
    ...
    $all('#lpTabs .vp-tab').forEach(function (tab) {
      tab.addEventListener('click', function () {
        ...
        lpQuiz = null;
        rsHideNotice('#lpContent');
        rsSet('listen:mode', lpMode === 'meaning' ? null : lpMode);
    ...
    $all('.tp-dir-btn').forEach(function (btn) {
      btn.addEventListener('click', function () {
        ...
        tpQuiz = null;
        rsHideNotice('#tpContent');
        rsSet('translate:dir', tpDirection === 'zh2vi' ? 'zh2vi' : null);
